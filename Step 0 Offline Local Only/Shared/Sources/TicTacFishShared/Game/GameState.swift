...
/// Represents a game move performed in the game of TicTacFish.
public struct GameMove: Hashable, Sendable {
    /// Identity of the player actor who performed the move.
    public let playerID: ActorIdentity
    /// Position on which the move was placed.
    /// The game field is indexed numerically from 0 to 8.
    public let position: Int // between 0 and 8

    /// Which team the move was made by.
    /// The player identified by `playerID` is playing for this `team`.
    public let team: CharacterTeam
    /// Which concrete character emoji should be used to mark the move on the field.
